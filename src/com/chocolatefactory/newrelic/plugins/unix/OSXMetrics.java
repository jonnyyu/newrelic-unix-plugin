...
public class OSXMetrics extends UnixMetrics {
	...
	public OSXMetrics() {
		...
		dfMapping.put(Pattern.compile("\\s*\\S+\\s+(\\d+)\\s+(\\d+)\\s+(\\d+)\\s+(\\d+)%\\s+\\d+\\s+\\d+\\s+\\d+%\\s+(\\S+).*"),
                  new String[]{"1K-blocks", "Used", "Available", "Use%", kColumnMetricDiskName});
		allCommands.put("df", new UnixCommand(new String[]{"df","-k", "-l"}, commandTypes.REGEXDIM, defaultignores, 0, dfMapping));
